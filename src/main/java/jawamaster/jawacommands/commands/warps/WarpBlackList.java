 ...
 * @author alexander
 */
public class WarpBlackList implements CommandExecutor{
...
//        }
        ...
        if (warp.playerIsWhiteListed(target)){
            warp.addToBlackList(target.getUniqueId());
        } else {
            warp.removeFromBlacklist(target.getUniqueId().toString());
        }
...
//        Set<UUID> addToBL = new HashSet();
//        
//        for(Item item : response.getResponses()) {
//            if (item.isFailure()){
//                failed.add(item.getResponse());
//            } else {
//                addToBL.add(UUID.fromString(item.getResponse().getHits().getHits()[0].getId()));
//            }
//        }
//        
//        warp.addToWhiteList(addToBL);
//        
        WarpHandler.updateWarp(warp);
        commandSender.sendMessage(ChatColor.GREEN + " > Adding " + args[1] + " to Blacklist.");
        return true;
